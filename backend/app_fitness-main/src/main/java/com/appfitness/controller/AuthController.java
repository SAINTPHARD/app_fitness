 ...
 * Expondo um endpoint POST /auth/login para receber as credenciais e devolver
 * um Token JWT. O processo de autenticação é o seguinte: 1. O cliente envia um
 * POST /auth/login com um JSON contendo "email" e "senha". 2. O controlador
 * tenta autenticar as credenciais usando o AuthenticationManager 3. Se a
 * autenticação for bem-sucedida, o TokenService gera um Token JWT assinado para
 * o usuário autenticado. 4. O controlador devolve um status 200 OK com o Token
 * JWT dentro de um TokenDTO. 5. Se a autenticação falhar (e-mail ou senha
 * errados), o controlador captura a AuthenticationException e devolve um status
 * 401 Unauthorized.
 */
@RestController
@RequestMapping("/auth")
public class AuthController {
	 ...
	 * @return ResponseEntity com o Token JWT em caso de sucesso ou mensagem de erro
	 *         em caso de falha
	 * 
	 * @Valid: Valida os campos do LoginDTO (e-mail e senha) antes de processar a
	 *         requisição
	 */
	@PostMapping("/login")
	public ResponseEntity<?> login(@Valid @RequestBody LoginDTO loginDTO) {
		try {
			...
			// 2. O gerente de autenticação valida se a senha bate com o hash BCrypt do
			// banco
			Authentication authentication = authenticationManager.authenticate(authToken);

			// 3. Em caso de sucesso, pega o Usuário autenticado que estava dentro do motor
			// do Spring
